...
log_file_name = 'test_log_Env_'+ models_name + '.txt'
...
dataset = Env()
test_dataset = EnvSampler(dataset, 'validation')
    ...
    def test(self, dataset, num_vote=10):
        ...
        while last_min < num_vote:
            ...
            for batch_idx, batch_data in enumerate(test_dataloader):
                ...
                new_min = np.min(test_dataloader.dataset.min_possibility['validation'])
            ...
            if True:
                ...
                if int(np.ceil(new_min)) > 2:
                    ...
                    for i_test in range(num_val):
                        ...
                        test_file_name = os.path.join(dataset.prefix,  name.split("_label")[0], name)
                        test_data = read_ply(test_file_name)
                        xyz = np.vstack((test_data['x'], test_data['y'], test_data['z'])).T

                        cloud_colors = env_label_to_color[preds.astype(np.uint8)]
