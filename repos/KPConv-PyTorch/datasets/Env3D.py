...
class Env3DDataset(PointCloudDataset):
    """Class to handle Railway3D dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'Railway3D')
        ...
        self.ignored_labels = np.array([]) # train with other
        # self.ignored_labels = np.array([10]) # train without other

        # Dataset folder
        self.path = '/data1/mengfanteng/dataset/ENVPC_dataset/20241129_MYEnv_PC/'
        self.scene = None
        self.kp_data_path = '/data1/mengfanteng/dataset/ENVPC_dataset/RandLANet-format/'
        if not exists(self.kp_data_path):
            makedirs(self.kp_data_path)
        ...
        self.test_file_path_txt  = 'repos/filelist/env/test.txt'
        ...
        self.all_file_name = self.train_file_name + self.val_file_name + self.test_file_name

        # print('train_paths = ', self.train_paths)
        # print('val_paths = ', self.val_paths)
        # print('train', self.train_file_name)
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for Railway3D data: ', self.set)
        ...
        self.files = []
        if self.set == 'training':
            self.files = self.train_paths
        elif self.set in ['validation', 'ERF']:
            self.files = self.val_paths
        elif self.set == 'test':
            self.files = self.test_paths
        else:
            raise ValueError('Unknown set for Railway3D data: ', self.set)
        ...
        if self.set == 'training':
            self.cloud_names = self.train_file_name
        elif self.set in ['validation', 'ERF']:
            self.cloud_names = self.val_file_name
        elif self.set == 'test':
            self.cloud_names = self.test_file_name
    ...
    def prepare_Railway3D_ply(self):

        print('\nPreparing ply files')
        t0 = time.time()

        # Folder for the ply files
        ply_path = join(self.path, self.scene)
        ...
        for cloud_name in self.cloud_names:
            ...
            original_ply = read_ply(join(self.path, self.scene, cloud_name + '.ply'))
            ...
            cloud_colors = np.vstack((original_ply['red'], original_ply['green'], original_ply['blue'])).T
            ...
            if cloud_name in ['ajaccio_2', 'ajaccio_57', 'dijon_9']:
                ...
                write_ply(join(ply_path, cloud_name + '.ply'), [cloud_points, cloud_colors, labels], field_names)
    ...
    def load_subsampled_clouds(self):
        ...
        print('self.config.first_subsampling_dl = ', dl)
        # Create path for files
        tree_path = join(self.kp_data_path, 'input_{:.3f}'.format(dl))
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                intensity = np.vstack((data['intensity_r'], data['intensity_g'], data['intensity_b'])).T

                sub_colors = intensity
                ...
                sub_colors = np.hstack((sub_intensity, sub_intensity, sub_intensity))
                sub_labels = np.squeeze(sub_labels)
                # print('sub_points.shape = ', sub_points.shape)
                # print('sub_colors.shape = ', sub_colors.shape)
                ...
                write_ply(sub_ply_file,
                          [sub_points, sub_intensity[:,0], sub_intensity[:,1], sub_intensity[:,2], sub_labels],
...
class Env3DSampler(Sampler):
    """Sampler for Railway3D"""

    def __init__(self, dataset: Env3DDataset):
...
class Env3DCustomBatch:
    """Custom batch definition with memory pinning for Railway3D"""
...
def Env3DCollate(batch_data):
    return Env3DCustomBatch(batch_data)
